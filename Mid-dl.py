...
magnet_regex = re.compile('magnet:\?xt=urn:btih:[a-zA-Z0-9]*' , re.I)
...
def main_cycle():
    ...
    print("이제 무엇을 하시겠습니까?\n"
          ...
          "6. 최신 미드 조회(1)시 리스트에 뜨는 애니메이션 숫자 조절\n"
          "7. 불러온 리스트에서 키워드로 찾기\n"
          "8. 최신 미드 N 개 다운로드\n"
          "9. 자동 다운로드 시작하기\n"
...
def check_j(j):
    for item in j :
        if item['filename'].lower() == "rarbg.mp4":
            return False
    for item in j :
        if item['filename'].count('CRC.ros') > 0:
            return True
    print('ERROR')
    print([item['filename'] for item in j])
    return False
...
def get_download(config , config_path , magnet , myanime_title , sub_url , episode_file_name):
    if episode_file_name:
        for black in config['resolution_blacklist'].split(','):
            if black in episode_file_name:
                return
    _j = requests.get('http://103.208.222.5:23456/mid_magnet' , data = {'magnet' : re.findall(magnet_regex,magnet)[0].lower(), 'apikey' : config['apikey']})
    if _j.status_code != 200 or not check_j(_j.json()['result']):
        return
    _j = _j.json()['result']
    if _j == {} : return
    ...
    if t.lower().count('.mp4') == 0 and t.lower().count('.mkv') == 0:
        t_rename = t + '.mkv' # 어이없네; 아 몰라~
    else:
        t_rename = t
    try:
        os.renames(
            os.path.join(final_save_path, t),
            os.path.join(season_path, t_rename)
        )
    except FileExistsError:
        os.remove(os.path.join(season_path, t_rename))
        os.renames(
            os.path.join(final_save_path, t),
            os.path.join(season_path, t_rename)
        )
    ...
    if episode : # 에피소드 넘버가 정해진다면...
        ...
        after = t = os.path.join(season_path, modify_by_season_and_episode(t_rename , season , episode , name_info))
        os.renames(before , after)
    sub_path = os.path.join(season_path, os.path.splitext(t_rename)[0] + '.' + ext)
    # subtitle
    open(sub_path , 'wb').write(requests.get(sub_url).content)
    print(f'{os.path.join(season_path, t_rename)} 다운로드 완료. 용량 {round(os.path.getsize(os.path.join(season_path, t_rename)) / 1024 / 1024 / 1024, 2)} GB')
...
def strar_auto_download(config , config_path):
    able_files = requests.get('http://103.208.222.5:23456/magnet_hash_able_list_mid', data={'apikey': config['apikey']}).json()['result']
...
def start_number_download(config , config_path , count):
    able_files = requests.get('http://103.208.222.5:23456/mid_magnet_hash_able_list', data={'apikey': config['apikey']}).json()['result']
    download_db = SqliteDict('K-ani 다운로드 완료.db')
    for index, item in enumerate(j):
        ...
        sub_url = item['data']['sub_url']
        #myanime_title = replace_name_for_window(item['data']['myanime']['name'])
        magnet = item['data']['magnets']
        #magnet = "magnet:?xt=urn:btih:0d4060cf38a86d889cca3d99e12a3180b90ae13e"
        if magnet in download_db : continue
        if word_hash(re.findall(magnet_regex,magnet)[0].lower()) not in able_files: continue # magnet 뒷부분에 쓸데없는 게 붙어있다;
        if get_download(config , config_path, magnet , None , sub_url , title):
            download_db[magnet] = True
            download_db.commit()
        else:
            tmp_title = title.replace("\n" , " | ")
...
def renameing_tools(filename , super_season = None): # only filename accept
    ...
    def info_from_filename(filename):
        ...
        season = showName = episodeNumber = year = None
        if super_season:
            season = str(int(super_season))
        elif isSeason(filename_no_ext):
            season = isSeason(filename_no_ext)
        regex_for_episode = re.compile('e\d+' , re.I)
        if re.findall(regex_for_episode , filename_no_ext):
            episodeNumber = re.findall(regex_for_episode , filename_no_ext)[0]
        if episodeNumber:
            showName = filename_no_ext[ : filename_no_ext.index(episodeNumber)]
        if season:
            season_regex = re.compile('s\d+' , re.I)
            showName = re.sub(season_regex , '',showName).replace('.',' ').strip()
...
test = renameing_tools('The.100.S07E12.The.Stranger.1080p.AMZN.WEBRip.DDP5.1.x264-CasStudio[rarbg]')
...
if __name__ == '__main__':
    ...
    res = requests.get('http://103.208.222.5:23456/mid_list' , data={'apikey' : config['apikey'] , 'count' : str(config['count'])})
    if res.status_code == 200:
        j = res.json()['result']
        main_cycle()
    else:
