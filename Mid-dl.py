...
def get_download(config, config_path, magnet, myanime_title, sub_url, episode_file_name):
    if episode_file_name:
        for black in config['resolution_blacklist'].split(','):
            if black in episode_file_name:
                return
    _j = requests.get('http://103.208.222.5:23456/mid_magnet',
                      data={'magnet': re.findall(magnet_regex, magnet)[0].lower(), 'apikey': config['apikey']})
    ...
    print(
        f'{os.path.join(season_path, t_rename)} 다운로드 완료. 용량 {round(os.path.getsize(os.path.join(season_path, t_rename)) / 1024 / 1024 / 1024, 2)} GB')
...
def strar_auto_download(config, config_path):
    able_files = \
    requests.get('http://103.208.222.5:23456/magnet_hash_able_list_mid', data={'apikey': config['apikey']}).json()[
        'result']
    ...
    for index, item in enumerate(j):
        ...
        if check:
            ...
            if word_hash(
                    re.findall(magnet_regex, magnet)[0].lower()) not in able_files: continue  # magnet 뒷부분에 쓸데없는 게 붙어있다;
...
def start_number_download(config, config_path, count):
    able_files = \
    requests.get('http://103.208.222.5:23456/mid_magnet_hash_able_list', data={'apikey': config['apikey']}).json()[
        'result']
    download_db = SqliteDict('K-ani 다운로드 완료.db')
    for index, item in enumerate(j):
...
def renameing_tools(filename, super_season=None):  # only filename accept
    ...
    if info == None: return
    tvdb_info = \
    requests.get('http://103.208.222.5:23456/tvdb_search', data={'title': info['title'], 'year': info['year']}).json()[
        'result']
    # myanimeinfo
...
if __name__ == '__main__':
    ...
    res = requests.get('http://103.208.222.5:23456/mid_list',
